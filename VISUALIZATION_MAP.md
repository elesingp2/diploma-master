...
| `fig:thermalRadialRelaxation` | `figures/fig14_thermal_radial_relaxation.pdf`, `figures/fig14_thermal_radial_relaxation.png` | (был в `main.tex` до реструктуризации; сейчас не подключён) | `scripts/make_fig_thermal_radial_relaxation.py` | Численное решение радиального уравнения теплопроводности в твэле (UO2 + He gap + Zr) после импульса мощности. (а) профили `ΔT_f(r,t)` с метками `√(α_f t)`; (б) разделение энергии между топливом, оболочкой и водой в log-времени, окна `K_D` и `H_fw`. |
| `fig:memoryConvolution` | `figures/fig15_memory_convolution.pdf`, `figures/fig15_memory_convolution.png` | `main.tex:178` (раздел «Модель памяти твэла») | `scripts/make_fig_memory_convolution.py` | Свёртка как память твэла: 4 панели — (а) вход `P(s)`; (б) ядро `K_D(τ)` с быстрой и медленной модами и `∫K_D = κ_D^st`; (в) снимок свёртки в момент `t*` с отражённым ядром, подынтегральным произведением и зоной причинности; (г) сравнение `ρ_D(t)` с памятью и квазистатики `−κ_D^st·P(t)`. |
